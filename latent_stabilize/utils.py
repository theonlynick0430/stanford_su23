...
def linear_action(env, target_state, update_target_state=None, thresh=0.05, max_steps=20, render=True):
# def linear_action(env, target_state, update_target_state=None, thresh=0.005, max_steps=20, render=True):
...
def single_action(env, target_state, update_target_state=None, thresh=0.005, max_steps=20, render=True):
    obs = env._get_observations()
    state = get_current_state(obs)
    error = np.linalg.norm(np.concatenate(target_state)-np.concatenate(state))
    steps = 0
